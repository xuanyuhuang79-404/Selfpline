...
## 下一轮：Dashboard 深度重构、Records 扩展与视觉精修

1. [x] Dashboard 删除重复图表模块。
2. [x] Dashboard 新增 Executive Hero 状态总览。
3. [x] Dashboard 新增今日执行中心。
4. [x] Dashboard 新增最近 3 条身体记录模块。
5. [x] Dashboard 新增今日记录状态模块。
6. [x] Dashboard 新增 AI 今日建议模块。
7. [x] Dashboard 新增计划健康度 / 待复盘模块。
8. [x] Dashboard 社区摘要改为轻量横幅。
9. [x] Records 新增步数、心情、精力、压力。
10. [x] Analytics 新增 BMI、平均步数、心理状态指标。
11. [x] 计划删除改为物理删除并修复完成率异常。
12. [x] “归档”文案改“结束”。
13. [x] 全局按钮、表单、卡片、emoji tile 视觉统一。
14. [x] 编译和基础功能验证。

## 下一轮：创建计划 AI 修复、Records 历史重构、Analytics 深度升级与 Profile 扩展

1. [ ] 创建计划页面统一 Build / Quit 创建入口。
2. [ ] 修复 AI 计划生成 planReady 触发异常。
3. [ ] 保证所有计划类型可正常跳转生成。
4. [ ] 修正中文提示词与状态文案。
5. [ ] Records 步数改为统一滑动栏交互。
6. [ ] Records 新增锻炼时长 exerciseMinutes。
7. [ ] 修复今日记录保存报错。
8. [ ] 删除 Records 历史趋势区域。
9. [ ] Records 历史记录支持按日期查看。
10. [ ] Dashboard 保存记录后状态同步更新。
11. [ ] Analytics 新增步数趋势图。
12. [ ] Analytics 新增睡眠趋势图。
13. [ ] Analytics 新增净摄入能量图。
14. [ ] Analytics 新增体重趋势图。
15. [ ] Analytics 新增锻炼时长趋势图。
16. [ ] Analytics BMI 改为最近体重 + Profile 身高计算。
17. [ ] Profile 删除健康目标字段展示。
18. [ ] 注册新增身高、体重、病史必填。
19. [ ] Profile 最新体重随每日记录自动更新。
20. [ ] Profile 页面新增非重复信息模块并优化排版。
21. [ ] Plans 筛选控件改为主题化 UI。
22. [ ] Dashboard 今日计划横幅支持左右切换。
23. [ ] 编译和基础功能验证。
